    ...
    for node in nx.topological_sort(G):
        ...
        if not parents:
            ...
            for parent in parents:
                true_coefficients[node][parent] = edge_weights[(parent, node)]
...
def run_simulation_with_seeds_custom(G, num_samples,
                                     X_columns, Y_column,
                                     I_columns=None, N_columns=None,
                                     B_columns=None, num_seeds=10):
    deviations = []
    for seed in range(num_seeds):
        ...
        I = data[I_columns].to_numpy() if I_columns is not None else None
        N = data[N_columns].to_numpy() if N_columns is not None else None
        B = data[B_columns].to_numpy() if B_columns is not None else None
        
        res = civ(X=X, Y=Y, I=I, N=N, B=B)
        
        deviation_X = abs(res[0] - coefficients[Y_column][X_columns])
        deviation_Z = abs(res[1] - coefficients[Y_column][N_columns[0]]) if N_columns is not None else None
        
        deviations.append((deviation_X, deviation_Z))
        values_X = np.array([item[0][0] for item in deviations])
        values_Z = np.array([item[1][0] for item in deviations]) if N_columns is not None else None

        mean_X = values_X.mean()
        std_X = values_X.std()
        
        mean_Z = values_Z.mean() if N_columns is not None else None
        std_Z = values_Z.std() if N_columns is not None else None
    return mean_X, std_X, mean_Z, std_Z

def simulate_linear_VAR(alpha, beta, gamma, theta, delta, T, burn_in=1000, add_edge=False):
