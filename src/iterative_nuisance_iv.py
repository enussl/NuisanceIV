...
class IterativeNIVStrategyFinder:
    def __init__(
            self,
            graph: nx.DiGraph,
            X: Set[str],
        ...
        """
        Initializes the IterativeNIVStrategyFinder class.
    ...
    def dfs_paths(self, graph, start, end, path=None):
        ...
        Returns:
        - List of all paths from start node to end node.
        """
        if path is None:
            path = []
        path = path + [start]
    ...
    def find_adjustment_sets_for_backdoor_nodes(self) -> Dict[
        str, Union[Set[str], List[Dict[str, Union[Set[str], Dict[str, Set[str]]]]], nx.DiGraph]
    ]:
        """
        ...
        for subset in list(self.powerset(self.backdoor_nodes)):
            adjustment_finder = NIVStrategyFinder(
                self.graph, set(subset), self.Y, self.hidden_nodes, self.type_effect, one_suffices=True
            )
            adjustment_sets = adjustment_finder.find_adjustment_sets_nuisance()
            if adjustment_sets:
                valid_backdoor_nodes.update(subset)
                self.adjustment_sets.append(
                    {
                        "Backdoor Set": set(subset),
        ...
        final_adjustment_finder = NIVStrategyFinder(
            self.graph, self.X, self.Y, self.hidden_nodes, self.type_effect, one_suffices=True
        )
...
class EstimateIterativeNIV:
    def __init__(
        ...
        X: Set[str], 
        Y: str, 
        gba_strategy: Dict[str, Set[str]], 
        final_strategy: Dict[str, Set[str]], 
        W: np.ndarray
    ):
        """
    ...
    def estimate_results(
        ...
        one_suffices: bool
    ) -> Tuple[Dict[str, Dict[str, Union[np.ndarray, str]]], Dict[str, Dict[str, float]], Dict[str, Dict[str, Union[float, str]]]]:
        """
        ...
        adjustment_set_finder = NIVStrategyFinder(G, self.X, self.Y, hidden_nodes, type_effect, one_suffices)
